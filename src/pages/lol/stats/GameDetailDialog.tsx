...
import type { MatchupDto, TeamGameDetailDto, TeamGameDto, TeamGamePlayerDto } from "../../../types/stats";
import { GameReviews } from "../reviews/GameReviews";
import { useRankGapLabel, useRankLabel } from "./rank";
...
export function GameDetailDialog({ teamId, game, avatars, onClose }: GameDetailDialogProps) {
  ...
  useEffect(() => {
    ...
    getTeamGameDetailApi(teamId, matchId)
      ...
      .catch((loadError) =>
        vivant && setError(loadError instanceof Error ? loadError.message : t("detail.loadFailed")),
      );
    return () => {
      vivant = false;
    };
  ...
  return (
    <Dialog
      open={game !== null}
      title={
        game?.startedAt ? t("detail.title", { date: formatDate(new Date(game.startedAt)) }) : t("games.detail")
      }
...
function FaceAFace({
  ...
  const ecartDeRang = useRankGapLabel();
      ...
      <Stack spacing={0.25}>
        <Stack direction="row" spacing={0.5} wrap>
          <Chip
            ...
            tone={or > 0 ? "success" : or < 0 ? "error" : "neutral"}
            variant="outline"
            size="small"
          />
